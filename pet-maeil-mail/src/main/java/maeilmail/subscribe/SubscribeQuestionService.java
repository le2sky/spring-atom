...
class SubscribeQuestionService {
    ...
    public void verify(VerifyEmailRequest request) {
        ...
        log.info("임시 사용자 인증 요청 이메일 = {} 코드 = {}", request.email(), code);
        emailSender.sendMail(mailMessage);

        TemporalSubscriberStore.add(request.email(), code);
    }

    public void checkVerified(CheckVerifiedRequest request) {
        log.info("임시 사용자 인증 확인 요청 이메일 = {} 코드 = {}", request.email(), request.code());
        TemporalSubscriberStore.verify(request.email(), request.code());
    }

    @Transactional
    public void subscribe(SubscribeQuestionRequest request) {
        log.info("이메일 구독 요청 = {}", request.email());
